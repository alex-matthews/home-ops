...
- Three changes follow, each landing as its own pull request referencing
  issue [#1233][i1233] and carrying its own validation, drawn from the
  detailed plans in the private operational notes: retarget Konflate's MCP
  client to the in-cluster service and remove the endpoint's external
  routing, as one paired change; remove the metrics endpoint from Gatus's
  public route; and limit Kromgo's public responses to the badge format.
