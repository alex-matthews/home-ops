...
function apply_talos_config() {
    ...
    for node in ${nodes}; do
        ...
        if ! machine_config=$(bash "${ROOT_DIR}/scripts/render-machineconfig.sh" "${ROOT_DIR}/talos/${machine_type}.yaml.j2" "${node_file}") || [[ -z "${machine_config}" ]]; then
            exit 1
        fi
...
function apply_helm_releases() {
    log debug "Applying Helm releases using helmfile"
